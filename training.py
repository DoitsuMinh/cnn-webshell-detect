...
def compile_and_scan_yara(filematch):
    yarcat()
...
def create_network(seq_length, optimizer, learning_rate, loss):
    # maximum length of input is maximum length of string
    network = tflearn.input_data(shape=[None, seq_length], name='input')

    # CNN model, using 3 1D convolution functions of number 128 and length 3, 4, 5 to process the data
    network = tflearn.embedding(network, input_dim=80000, output_dim=128)
...
def train_model(x1_code, y1_label, x2_code, y2_label):
    ...
    # initial CNN object and train data
